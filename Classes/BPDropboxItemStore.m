...
BPDropboxItemStore *BPDropboxItemStoreSharedInstance = nil;

@interface BPDropboxItemStore()
@property(nonatomic, retain) DBRestClient *dropBoxRestClient;
@end


@implementation BPDropboxItemStore

@synthesize dropBoxRestClient;

+ (BPDropboxItemStore*)sharedInstance {
	if (BPDropboxItemStoreSharedInstance == nil) {
		BPDropboxItemStoreSharedInstance = [[[self class] alloc] init];
	}
	return BPDropboxItemStoreSharedInstance;
}

- (void)dealloc {
	[BPDropboxItemStoreSharedInstance release];
	[super dealloc];
}
